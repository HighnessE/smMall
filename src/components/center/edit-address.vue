    ...
    methods: {
      ...
      formcheck () {
        ...
        this.$http({
          ...
          that.$router.push({path: '/center/infomation/address'})
        })
      },
      GetDataFromBackend () {
        let that = this
        ...
        // 判断是新建地址还是修改地址
        if (adid) {
          this.headeritem = '编辑地址'
          this.$store.state.address.submitaddr.type = 1
          this.$http({
            method: 'post',
            url: global.Domain.centerUrl + 'editads',
            body: {
              'adid': adid
            },
            emulateJSON: true
            ...
            that.$store.state.address.submitaddr = res.additems
            that.$store.state.address.submitaddr.only = res.only
            that.$store.state.address.submitaddr.type = 1
            let valueId = name2value(that.submitaddr.partment.split(' '), this.addressData)
            that.changeAddress(valueId.split(' '))
          })
        } else {
          this.headeritem = '新建地址'
          this.$store.state.address.submitaddr.type = 0
          this.$store.commit('clearSubmitaddr')
