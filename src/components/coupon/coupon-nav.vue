  ...
  <nav>
    <div v-if="!ifcollapse">
      <div class="itemwrap">
        <ul class="navitemswrap">
          <li class="navitems navcontent" v-for="(item,index) in couponitems" @click="changeHandle(index)">
    ...
    <transition name="navlist">
      <div class="collapsewrap" v-show="ifcollapse">
        <ul class="couponitems">
          <li v-for="(item, index) in couponitems" @click="changePos(index)">{{ item.title }}</li>
        </ul>
      </div>
    </transition>
  </nav>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ifcollapse: state => state.coupon.ifcollapse
      })
    },
    methods: {
      ...mapMutations([
